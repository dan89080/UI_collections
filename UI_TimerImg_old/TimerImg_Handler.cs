...
namespace TimerImg_old
{
    public unsafe class TimerImg_old_Handler
    {
        ...
        public TimerImg_old_Handler(Bitmap grid)
        {
            my_Bitmap = grid;
        ...
        public void Draw_Pixel(int to_X, int to_Y, int from_X, int from_Y)//, Color color)
        {
            for (int argb = 0; argb < argb_size; ++argb)
                ...
                //my_bmpPtr[to_Y * my_bmpData.Stride + to_X * argb_size + argb] = GetArgb(color, argb);
                //ptr[Y * Stride + X * argb_size + argb] = GetArgb(color, argb);
            //  my_bmpPtr[draw_Y * my_bmpData.Stride + j + argb]
        }
        ...
        public void Draw_MiddleLine(Color color)
        {
            ...
            for (int j = 0; j < my_bmpData.Stride; j += 4)
            {
                ...
                //Draw_Pixel(j / 4, draw_Y, Color.FromArgb(200, 191, 231));
            }
            ///
            /// UnlockBits
            ///
        ...
        public void Draw_ShiftLeft()
        {
            ...
            for (int i = 0; i < Pic_Y; i++)
            {
                for (int j = 4; j < my_bmpData.Stride; j += 4)
                {
                    ...
                    //Draw_Pixel((j - 4) / 4, i, j / 4, i);
                    //y * my_bmpData.Stride + x * argb_size + offset

                    //for (int argb = 0; argb < argb_size; ++argb)
                    //    my_bmpPtr[i * my_bmpData.Stride + (j - 4) / 4 * argb_size + argb] =
                    //        my_bmpPtr[i * my_bmpData.Stride + j / 4 * argb_size + argb];
                        //my_bmpPtr[GetPtrArrayIdx((j - 4) / 4, i, argb)] = my_bmpPtr[GetPtrArrayIdx(j / 4, i, argb)];
        ...
        private double get_sin(double amp, double freq, double X_now, double X_Offset = 0, double Y_Offset = 0)
        {
    ...
    public class Graph_time2value
    {
        ...
        public Graph_time2value(List<int> values, int time_start, int time_end)
        {
        ...
        private void updatePaths()
        {
            ...
            for(int i = time_start, m = 0; i < time_end; i++, m++)
            {
                path.AddLine(i, values[m], i + 1, values[m + 1]);
            }
        }
        public void updateValues(int val, bool discharge)
        {
            if(discharge)
                values.Remove(0);
